...
import { mapTabToHoaDonDto } from '@/utils/orderMapping'
...
export const useOrderStore = defineStore('order', () => {
  ...
  const conflictResolver = createConflictResolver({
    ...
    businessRules: {
      ...
      trangThaiCongNo: (currentStatus, incomingStatus, context) => {
         const statusOrder = [
    'PHU_PHI',
    'DA_THU_PHU_PHI',
    'HOAN_PHI',
  ...
  const debtStatuses = ref([
  { value: 'HOAN_PHI', label: 'Hoàn phí', severity: 'info' },
  { value: 'PHU_PHI', label: 'Phụ phí', severity: 'warn' },
  { value: 'DA_HOAN_PHI', label: 'Đã hoàn phí', severity: 'success' },
  { value: 'DA_THU_PHU_PHI', label: 'Đã thu phụ phí', severity: 'success' },
  { value: 'HOAN_THANH_CONG_NO', label: 'Hoàn thành công nợ', severity: 'secondary' }
])
  ...
  const debtStatusMap = computed(() => {
  const map = {}
  debtStatuses.value.forEach(dstatus => {
    map[dstatus.value] = dstatus
  })
  return map
})
  ...
  const fetchOrders = async (page = 0, size = 20, filterParams = {}) => {
    ...
    const cacheKey = `orderList:${JSON.stringify(params)}`
    ...
    const optimisticUpdate = createOptimisticUpdate(
      ...
      async () => {
        ...
        try {
          ...
          if (response.success) {
            // Real-time sync will handle data updates automatically
            return response
          } else {
  ...
  const createOrderFromTab = async (tabId = null) => {
    ...
    // Prepare order data for API using shared utility (simple scenario - minimal options)
    const orderData = mapTabToHoaDonDto(tab, {
      source: 'orderStore.js'
    })

    // Debug logging for payment status determination (maintained from original)
    console.log('OrderStore createOrderFromTab - Payment method:', tab.phuongThucThanhToan, 'Payment status:', orderData.trangThaiThanhToan)
  ...
  const processRefund = async (orderId) => {
    ...
    try {
      const response = await orderApi.processRefund(orderId)
  ...
  const updatePaymentStatus = async (orderId, paymentStatus,debtStatus, note = '') => {
    loading.value = true
    error.value = null
    ...
    try {
      ...
      if (response.success) {
        ...
        toast.add({
          severity: 'success',
          summary: 'Thành công',
          detail: `Trạng thái Công nợ đã được cập nhật: ${statusLabel}`,
          life: 5000
        })
const statusDebtLabel = debtStatusMap.value[debtStatus]?.label || debtStatus
toast.add({
  severity: 'info',
  summary: 'Cập nhật công nợ',
  detail: `Trạng thái công nợ đã được cập nhật: ${statusDebtLabel}`,
  life: 5000
})
        return response.data
      } else {
