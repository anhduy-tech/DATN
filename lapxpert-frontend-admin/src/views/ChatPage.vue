  ...
  <div class="chat-page flex flex-col min-h-[calc(100vh-160px)]">
    <div class="flex flex-col lg:flex-row gap-4 flex-1">
      ...
      <div class="w-full lg:w-[70%] bg-white p-4 rounded-lg shadow-md flex flex-col">
        ...
        <!-- Danh sách tin nhắn -->
        <div
          class="flex-1 overflow-y-auto mb-4 border border-gray-200 rounded p-2 max-h-[calc(100vh-250px)]"
        >
          <div
            v-for="(msg, index) in currentMessages"
            :key="index"
            class="p-2 border-b border-gray-200 flex"
            :class="{ 'justify-end': msg.sender === username, 'justify-start': msg.sender !== username }"
          >
            <div
              class="max-w-[70%] flex"
              :class="{ 'flex-row-reverse': msg.sender === username }"
            >
              <img :src="getAvatar(msg.sender)" class="w-6 h-6 rounded-full mx-2" />
              <div
                class="p-2 rounded-lg"
                :class="{
                  'bg-blue-50': msg.sender === username,
                  'bg-gray-100': msg.sender !== username,
                  'bg-green-50': msg.isPrivate,
                }"
              >
                <div class="flex items-center">
                  <strong>{{ msg.sender }} ({{ formatTimestamp(msg.timestamp) }})</strong>
                  <span v-if="msg.isPrivate" class="ml-2 text-green-600 font-bold text-sm"
                    >[Riêng tư]</span
                  >
                </div>
      ...
      <div class="w-full lg:w-[30%] bg-white p-4 rounded-lg shadow-md overflow-y-auto">
        <div class="mb-3 flex justify-between items-center">
          <label class="block font-semibold text-lg"
            >Danh sách phòng chat ({{ chatStore.waitingRoomCount }})</label
          >
          <button
            class="bg-gray-500 text-white px-3 py-1 rounded hover:bg-gray-600"
            @click="chatStore.fetchWaitingRooms"
          >
            Làm mới
          </button>
        </div>
        ...
        <div v-else-if="chatStore.filteredRooms.length === 0" class="text-gray-500 text-center">
          Không có phòng chat nào đang hoạt động
        </div>
        <ul v-else class="space-y-2 max-h-[calc(100vh-150px)] overflow-y-auto">
          <li
            v-for="room in chatStore.filteredRooms"
            :key="room.sessionId"
            class="p-2 border border-gray-200 rounded cursor-pointer hover:bg-gray-100 flex justify-between items-center"
            :class="{ 'bg-blue-100': room.sessionId === sessionId }"
            @click="connectToRoom(room.sessionId)"
          >
            <span>Phòng: {{ room.sessionId }}</span>
            <button
              v-if="room.sessionId === sessionId"
              class="bg-red-500 text-white px-2 py-1 rounded text-sm hover:bg-red-600"
              @click.stop="chatStore.disableRoom(room.sessionId)"
            >
              Vô hiệu hóa
            </button>
          </li>
...
// Khởi động polling khi component được mount
onMounted(() => {
  chatStore.startPolling();
});

// Dừng polling và ngắt kết nối khi component bị hủy
onUnmounted(() => {
...
<style scoped>
/* Xóa class avatar không sử dụng */
</style>